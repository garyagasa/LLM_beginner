...
本系列是 [NLP-Beginner](legacy/README.md) 在大模型时代的重构版本，是一份**独立的入门教程**，面向有 Python 与深度学习基础的学习者。沿用原系列「任务渐进 × 每个 2-4 周」的节奏，按"熟悉 Transformer → 从零实现 mini-GPT → 指令微调与对齐 → RAG → 工具调用 Agent → Mini Coding Agent → Mini Harness Engineering"七个任务展开，技术栈对齐 2025-2026 的主流。
...
这本教材可以作为 llm-beginner 的**前置知识**配合阅读：教材讲清原理，本仓库的 7 个任务负责把原理动手跑通。两者都可独立完成，不强相关。
...
七个任务目录结构一致：每个任务下都有 `requirements.txt`（依赖）、`data/download.py`（下载数据 / 模型或生成夹具）、`eval/run.py`（自检脚本）和 `eval/tutor_prompt.md`（贴给大模型做代码 review 的提示词）。你的实现写在各任务的 `src/` 下，按该任务 README「实现约定」表里列出的类 / 函数签名导出——自检脚本正是按这些签名导入并评测你的代码，照着写才能被正确评分。
...
> ⚠️ 请在仓库内运行 `eval/run.py`：它依赖仓库根目录的 `_eval_harness.py`（七个任务共用的运行壳）。把单个任务目录拷到仓库外会导致自检无法 import。
...
| 七 Harness Engineering | `python data/download.py` | 离线生成故障注入 toy-repo；无需模型、GPU 或网络 |
...
> 详细资源、离线夹具和自检脚本见 [Project7_harness-engineering/](Project7_harness-engineering/)

给任务六的 Coding Agent 增加可靠执行壳。固定 Agent 本身，集中学习如何用策略、隔离、确定性验证、结构化反馈、持久状态、trace 和预算控制提升任务成功率并降低误报。
...
1. 参考
   ...
   4. SEAGym：自演化 Agent 的训练、验证、回放与成本评测环境
2. 核心组件
   1. `PolicyEngine`：路径与命令 allowlist
   2. `SandboxRuntime`：固定 cwd、timeout、输出捕获与工作区隔离
   3. `RunStore`：配置、JSONL trace 与验证证据持久化
   4. `AgentHarness`：`attempt → verify → feedback → retry/stop`
3. 实现要求
   1. 第一步使用 Fake Agent，不接模型，证明 Harness 闭环本身正确
   2. 每次 run 使用独立副本，禁止修改源仓库
   3. 只相信 pytest 等确定性 verifier，不相信 Agent 自报成功
   4. 对路径逃逸、非法命令、超时、异常和预算耗尽给出结构化结果
   5. 第二步再通过 adapter 接入任务六的真实 Coding Agent
4. 评测
   1. Fake Agent 首次失败、第二次根据反馈成功
   2. 源仓库污染率为 0
   3. `max_attempts` 达到后可靠停止，无 false positive
   4. trace 可检查每次尝试、验证与最终状态
   5. 进阶：更新 Harness 后跑 held-out 与 replay，拒绝过拟合或回归
5. 时间：2-3 周
...
| 模型贯通 | Qwen2.5 体系全程：0.5B（任务 2-3）→ 7B-Instruct（任务 4-5）→ Coder-7B（任务 6）；任务 7 先固定模型、单独评估 Harness |
| 现代性 | 覆盖 2025-2026 主流概念：RoPE、LoRA、DPO、RAG、ReAct、MCP、Skill、Subagent、CodeAct、Harness、Verifier、Trace、Replay |
